...
function build_acl()
{
    ...
    do
        if [ "$1"x = "func"x ]; then
            ...
            target_type="board"
            tool_chain=arm-v01c02-linux-musleabi-gcc
        elif [ "$1"x = "musl"x ]; then
            echo "toolchain is arm-v01c02-linux-musleabi-gcc"
            target_type="board"
            tool_chain=arm-v01c02-linux-musleabi-gcc
        elif [ "$1"x = "glibc"x ]; then
            echo "toolchain is arm-v01c02-linux-gnueabi-gcc"
            target_type="board"
            tool_chain=arm-v01c02-linux-gnueabi-gcc
        else
...
cd $SWITCH_CROSS_DIR
ORI_CROSS_COMPILE_CHAIN=$(readlink lib32)
...
bash $SWITCH_CROSS_LIB_DIR/switch_cross_lib.sh arm-v01c02-linux-musleabi
cd $CUR_DIR
...
# prepare_env
# cd $SWITCH_CROSS_LIB_DIR
# bash $SWITCH_CROSS_LIB_DIR/switch_cross_lib.sh arm-v01c02-linux-gnueabi
# cd $CUR_DIR
